...
async function _doRegisterToken(btn, silent, activeMessaging) {
  ...
  try {
    ...
    if (permission !== "granted") {
      if (btn) { btn.style.display = "flex"; btn.innerHTML = `${bellIcon}<span class="notif-text">Enable Notifications</span>`; }
...
function updateTTSButton() {
  ...
  if (ttsEnabled) {
    ...
    btn.innerHTML = `
      ...
      <span class="tts-label">TTS On</span>`;
    btn.style.cssText = `
...
async function clearChatHistory() {
  ...
  if (btn) { btn.disabled = true; btn.innerHTML = `<span class="clear-label">Clearing…</span>`; }
  try {
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = `<span class="clear-icon">🗑️</span><span class="clear-label">Clear</span>`; }
  }
}
